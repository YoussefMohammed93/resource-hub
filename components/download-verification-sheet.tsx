      ...
      const pollTaskStatus = async (retryCount = 0): Promise<void> => {
        try {
          ...
          if (!taskResponse.success) {
            throw new Error(
              taskResponse.error?.message || "Failed to get task status"
            );
          }
              ...
              import("sonner").then(({ toast }) => {
                toast.success(
                  ...
                  ) + ` (${filename})`,
                  {
                    duration: 5000,
                    description: t(
                      "download.verification.progress.fileSavedToDevice"
                    ),
                  }
                );
              });
                    ...
                    {verificationData.subscription === false ? (
                      ...
                      <div className="space-y-6">
                        <div className="flex items-center gap-3">
                          <Badge
                            variant={
                              subscriptionData.active ? "success" : "secondary"
                            }
                            className="px-3 py-1"
                          >
                          ...
                          <div className="space-y-3">
                            ...
                            <div className="bg-secondary/50 border border-secondary rounded-lg p-3 sm:p-4">
                              ...
                              <p className="text-sm text-secondary-foreground/70 mt-1">
                                {Math.ceil(
                                  (new Date(subscriptionData.until).getTime() -
                                    Date.now()) /
                                    (1000 * 60 * 60 * 24)
                                )}{" "}
                                {t(
